...
if( ! empty($_GET['q']))
{
	...
	if( ! $erred)
	{
		...
		if($_GET['deep_search'])
		{
			$search_topics = $link->prepare('SELECT id, time, replies, visits, headline FROM topics WHERE headline LIKE ? OR body LIKE ? ORDER BY id DESC LIMIT 50');
			$search_topics->bind_param('ss', $search_query, $search_query);
		}
		else
		{
			$search_topics = $link->prepare('SELECT id, time, replies, visits, headline FROM topics WHERE headline LIKE ? ORDER BY id DESC LIMIT 50');
			$search_topics->bind_param('s', $search_query);
		}
		$search_topics->execute();
		$search_topics->store_result();
		
		if($search_topics->num_rows > 0)
		{
			echo '<h4 class="section">Topics</h3>';
			
			$search_topics->bind_result($topic_id, $topic_time, $topic_replies, $topic_visits, $topic_headline);
		...
		if($_GET['deep_search'])
		{
			$search_replies = $link->prepare('SELECT replies.id, replies.parent_id, replies.time, replies.body, topics.headline, topics.time FROM replies INNER JOIN topics ON replies.parent_id = topics.id WHERE replies.body LIKE ? ORDER BY id DESC LIMIT 50');
			$search_replies->bind_param('s', $search_query);
			$search_replies->execute();
			$search_replies->store_result();
			
			if($search_replies->num_rows > 0)
			{
				$search_replies->bind_result($reply_id, $parent_id, $reply_time, $reply_body, $topic_headline, $topic_time);
