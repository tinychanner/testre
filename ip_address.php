...
if( ! empty($_POST['ban_length']))
{
	...
	if( ! $erred)
	{
		$ban_ip = $link->prepare('INSERT INTO ip_bans (ip_address, expiry, filed) VALUES (?, ?, UNIX_TIMESTAMP()) ON DUPLICATE KEY UPDATE expiry = ?, filed = UNIX_TIMESTAMP()');
		$ban_ip->bind_param('sii', $ip_address, $new_ban_expiry, $new_ban_expiry);
		$ban_ip->execute();
		$ban_ip->close();
...
// Check for ban.
$check_ban = $link->prepare('SELECT filed, expiry FROM ip_bans WHERE ip_address = ?');
$check_ban->bind_param('s', $ip_address);
$check_ban->execute();
$check_ban->bind_result($ban_filed, $ban_expiry);
$check_ban->fetch();
$check_ban->close();
...
// Get statistics.
$query = "SELECT count(*) FROM topics WHERE author_ip = '" . $link->real_escape_string($ip_address) . "';";
$query .= "SELECT count(*) FROM replies WHERE author_ip = '" . $link->real_escape_string($ip_address) . "';";
$query .= "SELECT count(*) FROM users WHERE ip_address = '" . $link->real_escape_string($ip_address) . "';";
...
if($ip_num_ids > 0)
{
	...
	$stmt = $link->prepare('SELECT uid, first_seen FROM users WHERE ip_address = ? ORDER BY first_seen DESC LIMIT 5000');
	$stmt->bind_param('s', $ip_address);
	$stmt->execute();
	$stmt->bind_result($id, $id_first_seen);
